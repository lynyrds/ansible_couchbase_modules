...
class Couchbase(object):
    def __init__(self, module):
        ...
        self.setup_compaction = module.params['setup_compaction']
    ...
    def setupCompaction(self):
        ...
        cmd = cbcli + ' setting-compaction ' + '-c '+  masters['cluster'] + ' --username ' + self.cb_admin + ' --password ' + self.admin_password + ' --compaction-db-percentage ' + self.db_compaction + ' --enable-compaction-parallel 1' + ' --metadata-purge-interval ' + self.tombstone
        
        rc, stdout, stderr = self.module.run_command(cmd)
        
        if rc == 0:
            changed = True
            msg = msg + " Compaction has been set up."
        else:
            failed = True
            msg = msg + stderr
...
### <-- Check functions ###
    ...
    def execute(self):
        ...
        # Too lazy to verify the compaction settings. Let's just set it always.
        if self.setup_compaction:
            failed,changed,msg = map(self.setupCompaction().get, ('failed','changed','msg'))
            return dict(failed=failed,changed=True,msg=msg)
...
def main():
    fields = dict(
        ...
        # Compaction settings
        setup_compaction=dict(required=False, default=False, type="bool"),
