...
# 2017-12-13: Refactor to make linter more happy (@vovinacci)
...
options:
  ...
  rebalance:
    description:
      - Rebalance the cluster (please use run_once: True in your task!).
        If a bucket exists, rebalance will be skipped. Use force: True to issue a rebalance anyway
    required: false
    default: false
  force:
    description:
...
class Couchbase(object):
    """Couchbase object definition and implementation"""
    ...
    @staticmethod
    def rename_first_node():
        """There is no return code, no nothing to be evaluated
        I just assume the node has been renamed OK"""
        os.system('/usr/bin/curl -X POST '
                  'http://localhost:8091/node/controller/rename -d hostname={} '
                  '> /dev/null 2>&1'.format(my_fqdn))
    ...
    def check_new(self):
        ...
        for node in self.nodes:
            ...
            try:
                all_orchestrators[node] = \
                    requests.post(url, data="node(global:whereis_name(ns_orchestrator))",
                                  auth=(self.cb_admin, self.admin_password)).text
            except:
                failed = True
        ...
        for node in self.nodes:
            url = "http://" + node + ":" + port + "/diag/eval"
            try:
                all_local_orchestrators[node] =\
                    requests.post(url, data="node(global:whereis_name(ns_orchestrator))").text
            except:
                failed = True
    ...
    def move_node(self):
        """Group 1 is the default group all servers are joined to.
        Right now it's not possible to verify the group your server is in
        as json output isn't working.
        As of now (CB 5.0 April build), errors are redirected to the stdout,
        but rc=1 is OK if the group already exists"""
        # TODO:
        #   - validate sterr once it's redirected correctly
        #   - once json output is working, read the group your server is in
        #     and move (or keep) it not assuming the default
        ...
        cmd = '{} group-manage --move-servers={}:{} --from-group="Group 1" --to-group={}' \
              ' --cluster=localhost --username={} --password={}'\
            .format(cbcli, my_fqdn, port, self.group, self.cb_admin, self.admin_password)
    ...
    def cluster_init(self):
        ...
        my_ok = move + init
        failed = bool(my_ok)
        changed = not bool(my_ok)

        return dict(rc=my_ok, failed=failed, changed=changed, msg=my_message)

    def join_node(self):
        """Join node to cluster"""
        failed, masters, _ = map(self.check_new().get, ('failed', 'orchestrators', 'msg'))
        sleep(5)
        services = self.get_my_services()
        my_services = ','.join(services)
        ...
        cmd = [
            ...
            '-c', masters['cluster'],
            '--server-add=' + my_fqdn,
            '--server-add-username=' + self.cb_admin,
            '--server-add-password=' + self.admin_password,
    ...
    def run_rebalance(self):
        """Run cluster rebalance"""
        failed = False
        changed = False

        cmd = '{} rebalance -c localhost --username={} --password={}'\
            .format(cbcli, self.cb_admin, self.admin_password)
    ...
    def createUser(self):
        ...
        rc, _, stderr = self.module.run_command(cmd)
    ...
    def manage_tls(self):
        ...
        os.system('/usr/bin/curl -X POST -u {}:{} '
                  'http://127.0.0.1:8091/diag/eval '
                  '-d "ns_config:set(ssl_minimum_protocol, \'{}\')"'
                  .format(self.cb_admin, self.admin_password, min_version))
    ...
    def manage_http_ui(self):
        ...
        os.system('/usr/bin/curl -X POST -u {}:{} '
                  'http://127.0.0.1:8091/diag/eval '
                  '-d "ns_config:set(disable_ui_over_http, {})"'
                  .format(self.cb_admin, self.admin_password, disabled))
    ...
    def check_rebalance(self):
        ...
        bucket-list prints errors on stdout -- I hope this is going to be
        fixed in the GA 5.0 build"""
        changed = False
        failed = False
        msg = "No buckets found, can rebalance"

        cmd = '{} bucket-list -c localhost --username={} --password={}'\
            .format(cbcli, self.cb_admin, self.admin_password)
        rc, stdout, _ = self.module.run_command(cmd)
    ...
    def check_bucket(self):
        ...
        if not self.bucket_name or not self.bucket_mem:
            ...
            except (requests.ConnectionError, requests.ConnectionError, requests.HTTPError,
                    requests.URLRequired, requests.TooManyRedirects, requests.Timeout) as err:
                failed = True
                msg = 'Connection to the Couchbase REST API has failed! ({})'.format(err)
    ...
    def execute(self):
        ...
        # Because there is no way how to verify user's password,
        # let's say it's always runs and always "changed".
        if self.create_user:
            failed, changed, msg = map(self.createUser().get, ('failed', 'changed', 'msg'))
            return dict(failed=failed, changed=True, msg=msg)

        # No way to check if autofailover is set or not,
        # so always return changed=True -- same for audit
        if self.auto_failover or not self.auto_failover:
            failed, changed, msg = map(self.manage_autofailover().get, ('failed', 'changed', 'msg'))
            return dict(failed=failed, changed=True, msg=msg)

        if self.audit_enabled or not self.audit_enabled:
            failed, changed, msg = map(self.manage_audit().get, ('failed', 'changed', 'msg'))
            return dict(failed=failed, changed=True, msg=msg)

        if self.restrict_tls or not self.restrict_tls:
            failed, changed, msg = map(self.manage_tls().get, ('failed', 'changed', 'msg'))
            return dict(failed=failed, changed=True, msg=msg)

        if self.http_ui_enabled or not self.http_ui_enabled:
            failed, changed, msg = map(self.manage_http_ui().get, ('failed', 'changed', 'msg'))
            return dict(failed=failed, changed=True, msg=msg)


def main():
    """Start the ball - main function"""
    fields = dict(
        # Common things
